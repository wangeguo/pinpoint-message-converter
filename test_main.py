...
import requests
from main import PinpointMessageConverter, convert
import traceback

redis = Redis(host='127.0.0.1', port=6379, db=0)

def _test_convert_span():
...
def test_handle():
    # Start flask server
    thread = threading.Thread(target=PinpointMessageConverter().run)
    thread.start()
    ...
    with open('./pinpoint-topic.msg', 'rb') as f:
        ...
        while i <= 1:
            message = read_line(f)
            try:
                res = requests.post('http://localhost:38064', data=message)
